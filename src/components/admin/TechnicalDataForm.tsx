...
import { Save, Calculator, Check, AlertCircle, Loader2 } from 'lucide-react';
...
export function TechnicalDataForm() {
  ...
  const handleSaveRow = async (projectId: string) => {
    ...
    try {
      ...
      setEditData((prev) => { const c = { ...prev }; delete c[projectId]; return c; });

      // Show saved
      setSavedRows((prev) => new Set(prev).add(projectId));
      setTimeout(() => setSavedRows((prev) => { const n = new Set(prev); n.delete(projectId); return n; }), 3000);
    } catch (e) {
      setErrorMsg(`Error al guardar ${projectId}: ${e}`);
    } finally {
      setSavingRows((prev) => { const n = new Set(prev); n.delete(projectId); return n; });
    }
  };

  const handleSaveAll = async () => {
    const ids = Object.keys(editData).filter((id) => editData[id] && Object.keys(editData[id]).length > 0);
    for (const id of ids) {
  ...
  const getDisplayValue = (projectId: string, field: string, storedValue: number | undefined): string => {
    if (editData[projectId]?.[field] !== undefined) return String(editData[projectId][field]);
    return storedValue !== undefined ? String(storedValue) : '';
  };

  const hasChanges = Object.keys(editData).some((id) => editData[id] && Object.keys(editData[id]).length > 0);
  ...
  return (
    <Card>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                Edite los campos y presione <b>Guardar</b> en cada fila. Los datos se guardan en Firebase y se ven en cualquier navegador.
              </p>
            </div>
          </div>
          {hasChanges && (
      ...
      <CardContent className="overflow-x-auto">
        ...
        {errorMsg && (
          <div className="mb-3 p-2 bg-red-50 border border-red-200 rounded-lg text-xs text-red-700">{errorMsg}</div>
        )}
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr>
              ...
              <th className="py-1 px-1 text-center text-xs font-bold text-white rounded-t-md" style={{ backgroundColor: '#c0392b' }} colSpan={2}>ENTRADA MANUAL</th>
              <th className="py-1 px-1 text-center text-xs font-bold text-white rounded-t-md" style={{ backgroundColor: '#7f8c8d' }} colSpan={2}>CALCULADO</th>
              <th className="py-1 px-1 text-center text-xs font-bold text-white rounded-t-md" style={{ backgroundColor: '#d4a017' }}>MANUAL</th>
              <th className="py-1 px-1 text-center text-xs font-bold text-white rounded-t-md" style={{ backgroundColor: '#7f8c8d' }}>CALC.</th>
              <th className="py-1 px-1 text-center text-xs font-bold text-gray-600 border-b-2" rowSpan={2} style={{ width: '80px' }}>Accion</th>
            </tr>
          ...
          <tbody>
            {projects.map((project) => {
              ...
              const isSaving = savingRows.has(project.id);
              const rowHasEdits = editData[project.id] && Object.keys(editData[project.id]).length > 0;

              return (
                <tr key={project.id} className={`border-b border-gray-100 transition-all ${isSaved ? 'bg-green-50' : rowHasEdits ? 'bg-amber-50/50' : 'hover:bg-gray-50/50'}`}>
                  <td className="py-2 px-1">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-bold px-1.5 py-0.5 rounded shrink-0" style={{ color: '#273475', backgroundColor: '#273475' + '15' }}>{project.code}</span>
                  ...
                  <td className="py-1 px-1" style={{ backgroundColor: '#fdf2f2' }}>
                    <input type="number" step="0.01" min="0"
                      ...
                      className={`${inputClass} border-red-200 focus:ring-2 focus:ring-red-300/50 bg-white w-20`} placeholder="-" />
                  </td>
                  <td className="py-1 px-1" style={{ backgroundColor: '#fdf2f2' }}>
                    <input type="number" step="0.01" min="0"
                  ...
                  <td className="py-1 px-1" style={{ backgroundColor: '#fffdf0' }}>
                    <input type="number" step="1" min="0"
                      ...
                      className={`${inputClass} border-amber-200 focus:ring-2 focus:ring-amber-300/50 bg-white w-20`} placeholder="-" />
                  </td>
                  ...
                  <td className="py-1 px-1 text-center">
                    {isSaving ? (
                      <Loader2 className="w-4 h-4 animate-spin mx-auto" style={{ color: '#273475' }} />
                    ) : isSaved ? (
                      <span className="inline-flex items-center gap-1 text-green-600 text-xs font-bold">
                        <Check className="w-4 h-4" /> OK
                      </span>
                    ) : rowHasEdits ? (
                      <button onClick={() => handleSaveRow(project.id)}
                        className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-white text-xs font-bold hover:opacity-90"
